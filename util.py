...
class ParserHelper(UnitsContainer):
    ...
    def _from_string(cls, input_string):
        ...
        #ensure brackets (denoting a dimensionality) are ignored in tokenizer/build_eval_tree
        if '[' in input_string:
        ...
        #ensure braces (denoting a role annotation) are ignored in tokenizer/build_eval_tree
        if '{' in input_string:
            input_string = input_string.replace('{', '__ocbr__').replace('}', '__ccbr__')
            role = True
            #? should the ParserHelper instance build / hold the .role() annotations?
        else:
            role = False
        ...
        if not reps and not role:
            return ret
        elif reps and not role:
            return ParserHelper(ret.scale,
                            udict((key.replace('__obra__', '[').replace('__cbra__', ']'), value)
                                 for key, value in ret.items()))
        else: #both are true
            #todo alternative ParserHelper(RolesContainer) 
                # -> return both UnitsContainer & RolesContainer, properly assigned
                #? how?
            return ParserHelper(ret.scale,
                            udict((key.replace(
                                '__obra__', '[').replace(
...
class RolesContainer(Mapping):
    """Subclasses in order to return a UnitsContainer for Unit -> Role mapping;
    Overrides __init__ to enforce a 2-tuple of the mapping 
    i.e. unit : (exponent, role annotation)
    OMFG UnitsContainer does not get inherited, it IS the key for this mapping
    
    Planned behavior:
    >>> a = ureg.Quantity(1, 'm{}').role('member length')
    >>> a.roles
    <RolesContainer(('meter', 1), 'member length')>
    >>> b = RolesContainer(('m', 1), 'member length')
    <RolesContainer(('meter', 1), 'member length')>
    >>> c = ureg.Quantity(1, b)
    >>> a == c
    True
    ...
    maybe better to rewrite the mapping instead, and add @classmethod to 
    translate to a UnitsContainer, eg. pass RolesContainer instead of
    UnitsContainer to Quantity constructor
    
    TODO @classmethod to do this? or at the Quantity level?
    @classmethod appropriate bc this is read-only, we are returning a new
    instance from the *passed* in values
    """
    ...
    def __init__(self, *args):
        ...
        self._d = d
        for key, value in d.items():
            #todo instead, turn key into (unit, exp), values as annotation: str
            if not isinstance(key, tuple):
                raise TypeError('key must be Tuple(Union[str, Unit, UnitsContainer], int), not {}'.format(type(key)))
            if len(key) > 2:
                raise TypeError('key must be Tuple(Union[str, Unit, UnitsContainer], int), not {}'.format(type(key)))
            if not isinstance(key(0), [str, UnitsContainer]): #! class = Unit???
                raise TypeError('first element of key must be a str, Unit, or UnitsContainer, not {}'.format(type(key(0))))
            if not isinstance(key(1), int):
                raise TypeError('second element of key must be an int = unit exponent, not {}'.format(type(key(1))))
            if not isinstance(value, string_types):
                raise TypeError('value must be a str, not {}'.format(type(value)))
            self._hash = hash(frozenset(self._d.items()))

    @classmethod
    def _reconstruct_unitscontainer(cls):
        #todo
        pass
