...
class TestRoleAnnotations(QuantityTestCase):
    """This test case checks for correct retention and printing of unit roles
    """
    reason = "role() method not yet working"

    def test_unit_order_retained_nonreduceable(self):
        #todo foolish - kwarg order is not retained
        ureg = self.ureg
        Q_ = ureg.Quantity
        w_u = UnitsContainer([('joule', 1), ('mole', -1)])
        x_u = UnitsContainer([('kilogram', 1), ('second', -1)])
        y_u = UnitsContainer(mole=1, ampere=-1)
        z_u = UnitsContainer(pascal=1, kelvin=-1)
        w = Q_(1.0, w_u)
        x = Q_(1.0, x_u)
        y = Q_(1.0, y_u)
        z = Q_(1.0, z_u)
        
        self.assertEqual(repr(w), "<Quantity(1.0, 'joule / mole')>")
        self.assertEqual(repr(x), "<Quantity(1.0, 'kilogram / second')>")
        self.assertEqual(repr(y), "<Quantity(1.0, 'mole / ampere')>")
        self.assertEqual(repr(z), "<Quantity(1.0, 'pascal / kelvin')>")
    ...
    def test_unit_order_retained_reduceable(self):
        Q_ = self.ureg.Quantity
        aF_u = UnitsContainer([('meter', 2), ('newton', -1)])
        Fa_u = UnitsContainer([('newton', 1), ('meter', -2)])
        v_u = UnitsContainer([('meter', 3), ('meter', -3)])
        w_u = UnitsContainer(kilogram=1, meter=1, second=-2)
        x_u = UnitsContainer(newton=1)
        y_u = UnitsContainer(pascal=1, second=1)
        z_u = UnitsContainer(poise=1)
        aF = Q_(1.0, aF_u)
        Fa = Q_(1.0, Fa_u)
        V = Q_(1.0, v_u)
        w = Q_(1.0, w_u)
        ...
        z1 = Q_(1.0, UnitsContainer([('kilogram * meter / second ** 2', 1), ('newton',-1)]))
        z2 = Q_(1.0, UnitsContainer([('newton',1), ('kilogram * meter / second ** 2', -1)]))
        
        self.assertEqual(w, x)
        self.assertEqual(y, z)
        self.assertEqual((w/x).dimensionality, UnitsContainer())
        self.assertEqual((y/z).dimensionality, UnitsContainer())
        #self.assertEqual(z1.dimensionality, UnitsContainer())
        #self.assertEqual(z2.dimensionality, UnitsContainer())
        self.assertEqual(repr(aF), "<Quantity(1.0, 'meter ** 2 / newton')>")
        self.assertEqual(repr(Fa), "<Quantity(1.0, 'newton / meter ** 2')>")
        #self.assertEqual(repr(V), "<Quantity(1.0, 'meter ** 3 / meter ** 3')>")
        self.assertEqual(repr(y/z), "<Quantity(0.1, 'pascal * second / poise')>")        
        self.assertEqual(repr(z1), "<Quantity(1.0, 'kilogram * meter / second ** 2 / newton')>")
        self.assertEqual(repr(z2), "<Quantity(1.0, 'newton / kilogram * meter / second ** 2')>")

    def test_braces_ignored_ifno_role(self):
        """test that curly braces ignored when role() not called
        TODO should they be put back in place?
        """
        ureg = self.ureg
        x = ureg.Quantity(1, 'meter{}')
        y = ureg.Quantity(1, 'gram{}')
        z = ureg.Quantity(1, 'second{}')
        self.assertEqual(x.magnitude, 1)
        self.assertEqual(x.units, UnitsContainer(meter=1))
        self.assertEqual(y.magnitude, 1)
        self.assertEqual(y.units, UnitsContainer(gram=1))
        self.assertEqual(z.magnitude, 1)
        self.assertEqual(z.units, UnitsContainer(second=1))

    @unittest.skip(reason)
    def test_annotate_w_braces_single_unit(self):
        """TODO test that single unit roles are preserved and retrievable
        set unit to annotate by curly braces
        """
        ureg = self.ureg
        annotation_l = "member length" # [length] member length
        annotation_m = "salt" # [mass] salt
        annotation_t = "test run" # [time] test run
        x = ureg.Quantity(1, 'meter{}').role(annotation_l)
        y = ureg.Quantity(1, 'gram{}').role(annotation_m)
        z = ureg.Quantity(1, 'second{}').role(annotation_t)
        self.assertEqual(x.roles.values(),annotation_l) #? what will the key be
        self.assertEqual(y.roles.values(), annotation_m)
        self.assertEqual(z.roles.values(), annotation_t)
        self.assertRaises(KeyError, x.role[1])
        self.assertRaises(KeyError, y.role[1])
        self.assertRaises(KeyError, z.role[1])
    
    @unittest.skip(reason)
    def test_annotate_direct_registry_single_unit(self):
        """TODO test that single unit roles are preserved and retrievable
        set unit by direct registry call, infer annotation (by position | dim)
        """
        ureg = self.ureg
        annotation_l = "member length" # [length] member length
        annotation_m = "salt" # [mass] salt
        annotation_t = "test run" # [time] test run
        x = ureg.Quantity(1, ureg.meter).role(annotation_l)
        y = ureg.Quantity(1, ureg.gram).role(annotation_m)
        z = ureg.Quantity(1, ureg.second).role(annotation_t)
        self.assertEqual(x.roles.values(),annotation_l) #? what will the key be
        self.assertEqual(y.roles.values(), annotation_m)
        self.assertEqual(z.roles.values(), annotation_t)
        self.assertRaises(KeyError, x.role[1])
    ...
    @unittest.skip(reason)
    def test_annotate_w_braces_single_unit_repr(self):
        """TODO test __repr__ matches with single unit roles, direct units
        set unit to annotate by curly braces
        """
        ureg = self.ureg
        annotation_l = "member length" # [length] member length
        annotation_m = "salt" # [mass] salt
        annotation_t = "test run" # [time] test run
        repr_l = "<Quantity(1.0, 'meter member length')>"
        #repr_l = "<Quantity(1.0, 'meter', 'meter member length')>"
        repr_m = "<Quantity(1.0, 'gram salt')>"
        #repr_m = "<Quantity(1.0, 'gram', 'gram salt')>"
        repr_t = "<Quantity(1.0, second test run')>"
        #repr_t = "<Quantity(1.0, 'second', 'second test run')>"
        x = ureg.Quantity(1, 'meter{}').role(annotation_l)
        y = ureg.Quantity(1, 'gram{}').role(annotation_m)
        z = ureg.Quantity(1, 'second{}').role(annotation_t)
        self.assertEqual(repr(x), repr_l)
        self.assertEqual(repr(y), repr_m)
        self.assertEqual(repr(z), repr_t)    
    
    @unittest.skip(reason)
    def test_annotate_direct_registry_single_unit_repr(self):
        """TODO test __repr__ matches with single unit roles, direct units
        set unit by direct registry call, infer annotation (by position | dim)
        """
        ureg = self.ureg
    ...
    @unittest.skip(reason)
    def test_annotate_w_braces_nonreduceable_units(self):
        """TODO test that non-reduceable unit roles are preserved and retrievable
        set unit to annotate by curly braces
        only mass / mole flux for now
        """
        ureg = self.ureg
        annotation = ("salt",) # [mass] or [substance] salt
        x = ureg.Quantity(1, "kilogram{} / (meter**2 * second)").role(annotation)
        y = ureg.Quantity(1, "mole{} / (meter**2 * second)").role(annotation)
        self.assertEqual(x.roles[0], annotation)
        self.assertEqual(y.roles[0], annotation)
        self.assertRaises(KeyError, x.roles[1] and x.roles[2] == '')
        self.assertRaises(KeyError, y.roles[1] and y.roles[2] == '')
    
    @unittest.skip(reason)
    def test_annotate_direct_registry_nonreduceable_units(self):
        """TODO test that non-reduceable unit roles are preserved and retrievable
        set unit by direct registry call, infer annotation (by position | dim)
        only mass / mole flux for now
        """
        ureg = self.ureg
        mass_units = UnitsContainer(kilogram=1, meter=-2, second=-1)
        mole_units = UnitsContainer(mole=1, meter=-2, second=-1)
        annotation = ("salt",) # [mass] or [substance] salt
        x = ureg.Quantity(1, ureg.kilogram / ureg.meter**2 / ureg.second).role(annotation)
        y = ureg.Quantity(1, ureg.mole / ureg.meter**2 / ureg.second).role(annotation)
        self.assertEqual(x.roles.values(), annotation)
        self.assertEqual(y.roles.values(), annotation)
        self.assertRaises(KeyError, x.roles[('meter',1)])
        self.assertRaises(KeyError, y.roles[1] and y.roles[2] == '')
           
    @unittest.skip(reason)
    def test_annotate_w_braces_nonreduceable_units_repr(self):
        """TODO test __repr__ matches with non-reduceable unit roles 
        set unit to annotate by curly braces
        only mass / mole flux for now
        """
        ureg = self.ureg
        ...
        x = ureg.Quantity(1, "kilogram{} / (meter**2 * second)").role(annotation)
        y = ureg.Quantity(1, "mole{} / (meter**2 * second)").role(annotation)
        repr_x = "<Quantity(1.0, 'kilogram salt / meter**2 / second)>"
        #repr_x = "<Quantity(1.0, 'kilogram / meter**2 / second', 'kilogram salt / meter**2 / second')>"
        repr_y = "<Quantity(1.0, 'mole salt / meter**2 / second)>"
        #repr_y = "<Quantity(1.0, 'mole / meter**2 / second', 'mole salt / meter**2 / second')>"
        self.assertEqual(repr(x), repr_x)
        self.assertEqual(repr(y), repr_y)       

    @unittest.skip(reason)
    def test_annotate_direct_registry_nonreduceable_units_repr(self):
        """TODO test __repr__ matches with non-reduceable unit roles 
        set unit by direct registry call, infer annotation (by position | dim)
        only mass / mole flux for now
        """
