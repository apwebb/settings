...
class _Quantity(PrettyIPython, SharedRegistryObject):
    ...
    def role(self, *args):
        #TODO get working for curly braces annotation in unit strings
        #TODO get working for inferred w/ direct registry calls for units
        #TODO get working for inferred w/ UnitsContainer passed in
        #! to ensure units don't auto-convert/reduce -> directly create a UnitsContainer
        #* then do all parsing here, or create additional parsers
        #* call those, then create the UnitsContainer + roles mapping desired
        #* This only works for units that are not exactly the same 
        #* eg. kg*m/s^2 / newton won't reduce; m^3 / m^3 will reduce(?)
        #? Roles mapping could be a UnitsContainer? -> Nope! A subclass of?
        # ? reparse units - feels silly, twice the work
        self._units = self._REGISTRY.parse_units(self._units, role=True)._units
        #todo this way we need to set a new @property for roles?
        self._roles = RolesContainer(self._units, *args)
        # ? we need something as keys for the role values
